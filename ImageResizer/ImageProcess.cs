    ...
    public class ImageProcess
    {
        ...
        public async Task ResizeImagesAsync(string sourcePath, string destPath, double scale)
        {
            ...
            foreach (var filePath in allFiles)
            {
                var task = Task.Run(() =>
                {
                    //Console.WriteLine($"執行緒 的 ID={Thread.CurrentThread.ManagedThreadId}");
                    ResizeImages(destPath, scale, filePath);
                });
        ...
        public void OldResizeImages(string sourcePath, string destPath, double scale)
        {
            var allFiles = FindImages(sourcePath);
            foreach (var filePath in allFiles)
            {
        ...
        private void ResizeImages(string destPath, double scale, string filePath)
        {
            Image imgPhoto = Image.FromFile(filePath);
            string imgName = Path.GetFileNameWithoutExtension(filePath);
